...
# threshold_slip = 0.13
# threshold_guess = 0.22
# threshold_slip =0.45
# threshold_guess =0.45
threshold_slip = 1
threshold_guess = 1
...
def acquireXForOneStudent(A, q_matrix, questions, knowledge, S, G):
    YITA = [0] * questions
    for j in range(questions):
        yita = 1
        for k in range(knowledge):
            yita = yita * pow(A[k], q_matrix[j][k])
        YITA[j] = yita

    X = [0] * questions
    for j in range(questions):
        x = pow(G[j], (1 - YITA[j])) * pow((1 - S[j]), YITA[j])
        if x >= 0.5:
            X[j] = 1
        else:
            X[j] = 0
...
def acquireSDX(student, question, YITA, slip, guess, A):
    studentI = []
    for i in range(len(A)):
        count = 0
        for k in range(len(A[0])):
            count += A[i][k]
        studentI.append(count)
...
def decode_slip(x, len_s_g):  # 失误率解码函数
    # x为失误率
    min_x = 0.0
    num_points = pow(2, len_s_g)
...
def decode_guess(x, len_s_g):  # 猜测率解码函数
    min_x = 0.0
    num_points = pow(2, len_s_g)
...
def acquirePreAnswer(individual, q_matrix, n_students, n_questions, n_knowledge, slip, guess, flag_train,
                     GENE_LENGTH, len_s_g):
...
def acquireDataIsESVE(data, sample_student):
    data_list = [i for i in range(len(data))]
    data_new = random.sample(data_list, sample_student)
    data_new.sort()
    data_new_all = []
    for i in range(len(data_new)):
        data_new_all.append(data[data_new[i]])
    # print(data_ESVE_id)
    # print(data_LS_id)

    return data_new, data_new_all


def updateIndividual_A_by_student(seed_LS, global_guide_ind, update_LS_id, n_knowledge):
    for i in range(len(update_LS_id)):
        seed_LS[update_LS_id[i] * n_knowledge: (update_LS_id[i] + 1) * n_knowledge] \
            = global_guide_ind[update_LS_id[i] * n_knowledge: (update_LS_id[i] + 1) * n_knowledge]
